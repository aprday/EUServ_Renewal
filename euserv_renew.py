#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Ported from dufei511/euserv_py (MIT) and adapted for this repository.
"""
...
class AccountConfig:
    ...
    def __init__(self, email, password, email_pin='', email_password='', imap_server=''):
        ...
        self.email_pin = (email_pin or self.email).strip()
        # Gmail 应用专用密码需要保留原格式（含空格）；只清理首尾空白。
        self.email_password = (email_password or password or "").strip()
...
class GlobalConfig:
    """全局配置"""
    def __init__(self, telegram_bot_token="", telegram_chat_id="", bark_url="", max_workers=3, max_login_retries=3, telegram_api_host="https://api.telegram.org"):
        self.telegram_bot_token = telegram_bot_token
        self.telegram_chat_id = telegram_chat_id
        self.bark_url = bark_url  # 新增：Bark 推送 URL
        self.max_workers = max_workers
        self.max_login_retries = max_login_retries
        self.telegram_api_host = (telegram_api_host or "https://api.telegram.org").rstrip("/")
...
GLOBAL_CONFIG = GlobalConfig(
    ...
    telegram_chat_id=os.getenv("TG_CHAT_ID") or os.getenv("TG_USER_ID"),  # TG 的 User ID
    ...
    telegram_api_host=os.getenv("TG_API_HOST") or "https://api.telegram.org",
)
...
def load_accounts_from_env() -> List[AccountConfig]:
    ...
    while True:
        ...
        email = os.getenv(f"EUSERV_EMAIL{suffix}") or os.getenv(f"EUSERV_USERNAME{suffix}")
        if not email or not email.strip():
            break
        password = os.getenv(f"EUSERV_PASSWORD{suffix}")
        email_pin = os.getenv(f"EMAIL_PIN{suffix}") or os.getenv(f"EMAIL_USERNAME{suffix}")
        email_password = os.getenv(f"EMAIL_PASS{suffix}") or os.getenv(f"EMAIL_PASSWORD{suffix}")
        accounts.append(AccountConfig(
            email=email,
            password=password,
            email_pin=email_pin,       # 可选，未配置则使用 EUSERV_EMAIL/EUSERV_USERNAME
            email_password=email_password,  # PIN 邮箱的密码（Gmail 应用专用密码等）
        ))
        i += 1
    return accounts
...
def _try_math_expression(raw_text: str) -> Optional[str]:
    """把 TrueCaptcha 返回的文本按算式/纯字母数字两种形式解析。"""
    ...
    match = re.match(r"^(\d+)([+\-*/])(\d+)$", corrected)
    if not match:
        return cleaned
    ...
    try:
        result = {
            ...
            "-": left - right,
            "*": left * right,
            "/": left // right if right and left % right == 0 else None,
        }.get(op)
    except ZeroDivisionError:
        result = None
    return str(result) if result is not None else cleaned


def _solve_captcha_api(image_bytes: bytes) -> Optional[str]:
    """调用 TrueCaptcha API 识别验证码；失败返回 None。"""
    if not (CAPTCHA_USERID and CAPTCHA_APIKEY):
        return None
    try:
        ...
        if result.get("status") == "error":
            logger.warning(f"TrueCaptcha API 返回错误: {result.get('message')}")
            return None
        captcha_text = result.get("result")
        if captcha_text:
            return _try_math_expression(str(captcha_text))
    except Exception as e:
        logger.warning(f"TrueCaptcha API 识别失败，回退本地 OCR: {e}")
    return None
...
def send_telegram(message: str, config: GlobalConfig):
    ...
    url = f"{config.telegram_api_host}/bot{config.telegram_bot_token}/sendMessage"
    data = {
