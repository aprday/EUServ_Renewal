...
- 根据每份合同在 EUserv 后台显示的可续期日期自动计算检查窗口
- 默认从可续期日期前 3 天开始，每天最多登录检查一次
- 续期成功后保存下一次可续期日期，在对应月份前不再登录
- 可续期日期前失败时通过 Telegram 报告原因并在次日重试
- 到达合同可续期日期仍失败时报告原因并暂停后续自动尝试
- 本地 `ddddocr` 识别验证码
...
GitHub Actions 每天 UTC 12:00，也就是北京时间 20:00，先读取本地 `renewal_state.json`。这一步只做日期判断，不会登录 EUserv。只有当天已经进入“合同可续期日期前 3 天至可续期日期”的窗口，才安装依赖并登录检查。

EUserv **没有所有用户统一的每月 9 日续期规则**。每份合同的可续期日期由服务器开通时间、合同周期和 EUserv 后台状态决定，应以合同页面显示的以下信息为准：

```text
Contract extension possible from YYYY-MM-DD
```

首次部署时，应把自己合同页面显示的日期写入 `renewal_state.json` 的 `next_renewal_date`。续期成功并验证日期变化后，脚本会自动保存下一周期的新日期，不需要每月手工修改。脚本只有在页面明确出现 `Extend contract` 控件时才提交续期。
...
这个示例合同的检查窗口才是 9 月 6–9 日：6、7、8 日若续期入口尚未开放则不提交续期，9 月 9 日达到该合同显示的可续期日期后继续验证。换成其他日期，窗口会自动随之变化，例如可续期日期为 11 月 23 日，则从 11 月 20 日开始检查。

续期成功后会生成当月 `renewed_YYYY-MM.txt`，并把下一周期日期写入 `renewal_state.json`；到下一个检查窗口前不再登录 EUserv。如果已经达到该合同的可续期日期，但仍出现登录失败、合同读取失败、续期入口未开放或续期无法验证等错误，脚本会发送 Telegram 原因通知，并将状态改为 `stopped`。此后自动任务不再登录，需人工排查后把状态改回 `waiting` 并填入正确的 `next_renewal_date`。

`renewal_state.json` 示例：

```json
{
  "next_renewal_date": "2026-11-23",
  "status": "waiting",
  "detail": "",
  "updated_at": "2026-08-24T20:00:00+08:00"
}
```
